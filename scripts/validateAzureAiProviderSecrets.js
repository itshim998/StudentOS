...
export function validateAzureAiProviderSecrets(env = {}) {
  ...
  const ok = fallbackReady && (!cycleEnabled || (groq.ok && geminiReady && pollinationsReady));
  return {
    ok,
    cycleEnabled,
    configuredProviders,
...
export function reportAzureAiProviderValidation(env = {}, output = console) {
  ...
  if (status.ok) {
    output.log(`AI provider configuration ready (providers=${status.configuredProviders.join(",")}, fallback=${status.fallbackReady}, cycle=${status.cycleEnabled}, Groq slots=${status.groqDistinctKeyCount}, Gemini slots=${status.geminiDistinctKeyCount}, Pollinations=${status.pollinationsConfigured}).`);
  } else {
