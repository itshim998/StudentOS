...
for (const heading of ["What to focus on", "Core lesson", "Study steps", "Quick self-check"]) assert.match(deterministic, new RegExp(heading));
...
const [html, app, server, css, packageJson] = await Promise.all([
...
const nav = html.slice(html.indexOf('aria-label="Views"'), html.indexOf('class="rail-session-card"'));
...
assert.match(server, /reserveAiWeeklyAllowance[\s\S]*workflow: "study_material"/);
