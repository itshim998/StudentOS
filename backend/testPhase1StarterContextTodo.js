...
assert.match(serverSource, /workflow: "daily_todo"/);
assert.match(serverSource, /isLogicalSuccess: \(result\) => result\?\.generationSucceeded/);
...
assert(todoRoute.indexOf("readiness.canGenerateTodo") < todoRoute.indexOf("executeAuthorizedAiOperation"));
...
assert.doesNotMatch(starterTodaySource, /(?:Provider|Model|Backend|Debug)\s+(?:status|details?|error|code)|OAuth scope/i);
