...
assert.match(rolloutWorkflow, /REQUEST_CONFIRMATION:\s*\$\{\{ inputs\.confirmation \}\}/);
assert.doesNotMatch(rolloutWorkflow, /confirmation="\$\{\{ inputs\.confirmation \}\}"/);
...
const darkDeploymentGateStart = rolloutWorkflow.indexOf("- name: Validate current dark deployment and worker topology");
const darkDeploymentGateEnd = rolloutWorkflow.indexOf("- name: Apply controlled rollout action", darkDeploymentGateStart);
assert(darkDeploymentGateStart >= 0 && darkDeploymentGateEnd > darkDeploymentGateStart);
const darkDeploymentGate = rolloutWorkflow.slice(darkDeploymentGateStart, darkDeploymentGateEnd);
assert.match(darkDeploymentGate, /for json in "\$api_json" "\$worker_json"/);
assert.match(darkDeploymentGate, /STUDENTOS_ADAPTIVE_RECOVERY_ENABLED" and \.value == "false"/);
assert.match(darkDeploymentGate, /STUDENTOS_RECOVERY_UI_ENABLED" and \.value == "false"/);
assert.match(darkDeploymentGate, /STUDENTOS_RECOVERY_ROLLOUT_MODE" and \.value == "off"/);
assert.match(darkDeploymentGate, /\.properties\.latestRevisionName == \.properties\.latestReadyRevisionName/);
